const fs=require('fs'); const path=require('path'); const dotenv=require('dotenv'); const {z}=require('zod');
...
function loadDotenvIfExists(p){ if(fs.existsSync(p)){ dotenv.config({path:p,override:false}); return true; } return false; }
function loadEnv(){
...
function doctor(){
  const r=loadEnv(); const blockers=[]; const warnings=[];
