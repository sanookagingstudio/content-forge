...
function ReadJson([string]$path){
  ...
  try { return ($raw | ConvertFrom-Json -Depth 99) } catch { throw "Invalid JSON: $path :: $($_.Exception.Message)" }
}
function WriteJson([string]$path,$obj){
  $json = $obj | ConvertTo-Json -Depth 99
...
function ProbeHttp([string]$url, [int]$tries=20, [int]$sleepSec=1){
...
$runDir = Join-Path $runBase ("ONEPACK_BASELINE_DEV_READY_{0}" -f $stamp)
EnsureDir $runDir
...
function RewriteDepsInObj($pkgObj, [string]$pkgPath, [hashtable]$localMap, [ref]$actions, [ref]$changed, [ref]$blockers){
  ...
  foreach($field in $fields){
    ...
    $names = @()
    if($deps -is [PSCustomObject] -and $null -ne $deps.PSObject -and $null -ne $deps.PSObject.Properties){
...
if(Test-Path $apiPkgPath){
  ...
  if(HasProp $apiPkg "dependencies"){
    $deps = GetProp $apiPkg "dependencies"
    if($null -ne $deps){
...
# -------------------- dev boot + probe --------------------
$dev = [ordered]@{ api_started=$false; api_pid=$null; web_started=$false; web_pid=$null; api_ok=$false; web_ok=$false; stopped=$false }
$apiProc = $null
$webProc = $null
$apiStdout = Join-Path $runDir "api.stdout.txt"
$apiStderr = Join-Path $runDir "api.stderr.txt"
$webStdout = Join-Path $runDir "web.stdout.txt"
$webStderr = Join-Path $runDir "web.stderr.txt"

if($blockers.Count -eq 0){
  try{
    Push-Location $repoRoot
    
    Write-Host ("[{0}] Starting dev servers separately..." -f (NowIso))
    
    # Start API
    $apiProc = Start-Process -FilePath "cmd.exe" -ArgumentList "/d","/s","/c","npm -w apps/api run dev > `"$apiStdout`" 2> `"$apiStderr`"" -WorkingDirectory $repoRoot -PassThru -WindowStyle Hidden
    if($apiProc){
      $dev.api_started=$true
      $dev.api_pid=$apiProc.Id
    }
    
    # Start Web
    $webProc = Start-Process -FilePath "cmd.exe" -ArgumentList "/d","/s","/c","npm -w apps/web run dev > `"$webStdout`" 2> `"$webStderr`"" -WorkingDirectory $repoRoot -PassThru -WindowStyle Hidden
    if($webProc){
      $dev.web_started=$true
      $dev.web_pid=$webProc.Id
    }
    
    Pop-Location
    
    if($dev.api_started -and $dev.web_started){
      Write-Host ("[{0}] Waiting for servers..." -f (NowIso))
      Start-Sleep -Seconds 10
      
      Write-Host ("[{0}] Probing API: http://localhost:4000/health" -f (NowIso))
      $dev.api_probe = ProbeHttp "http://localhost:4000/health" 20 1
      $dev.api_ok = [bool]$dev.api_probe.ok
      
      Write-Host ("[{0}] Probing Web: http://localhost:3000" -f (NowIso))
      $dev.web_probe = ProbeHttp "http://localhost:3000" 20 1
    ...
    Write-Host ("[{0}] Stopping dev processes..." -f (NowIso))
    try{
...
try{
  ...
  if($gitOk){
    ...
    if($pending -and ($pending | Where-Object { $_ -notmatch '^\s*$' })){
      & git commit -m "baseline: dev-ready (deterministic install + probes)" 2>&1 | Out-Null
...
if($status -eq "PASS"){
  ...
  if(Test-Path $apiStderr){
    $stderrTail = Get-Content $apiStderr -Tail 40 -ErrorAction SilentlyContinue
    if($stderrTail){
      $human += "--- API stderr tail (last 40 lines) ---"
  ...
  if(Test-Path $webStderr){
    $stderrTail = Get-Content $webStderr -Tail 40 -ErrorAction SilentlyContinue
    if($stderrTail){
      $human += "--- Web stderr tail (last 40 lines) ---"
      $human += ($stderrTail -join "`n")
      $human += ""
    }
  }
  ...
  if($npmDebug){
    $npmErrTail = Get-Content $npmDebug -Tail 40 -ErrorAction SilentlyContinue
    if($npmErrTail){
      $human += "--- npm debug log tail (last 40 lines) ---"
...
$md = @()
$md += "# ONEPACK_BASELINE_DEV_READY"
$md += ""
$md += "## Machine Report"
$md += ""
$md += "```json"
$md += ($machine | ConvertTo-Json -Depth 99)
$md += "```"
$md += ""
$md += "## Human Summary"
$md += ""
